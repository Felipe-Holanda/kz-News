    ...
    static async newHighlightNews() {
        ...
        for (let i = 0; i < newsData.length; i++) {
            ...
            if (newsData[i].id == randomId) {
                ...
                const newHighlight = new bigNews(newsData[i].imagem, newsData[i].categoria, newsData[i].titulo, newsData[i].resumo, newsData[i].fonte, newsData[i].noticia_completa);
